...
2026-09-06 实际执行：27 项状态行为测试通过；官方 Skill 校验在可编辑源、仓库发现副本、本机安装副本上均通过。15 个 Skill 文件的 SHA-256 在三个位置一致，Markdown 相对文件链接检查通过。未执行真实宿主任务创建/路由的端到端验证。

保留原有 16 项状态测试，新增 11 项，合计 27 项。执行命令见 README。新增覆盖：

- 无状态问答不初始化；草案问答不激活、不改写文件。
- 已有路线恢复断点，状态文件字节保持不变。
- 问题包保存原讲解和源码位置，主线推进后快照保持不变。
- 交接不改写主线/章节；重复交接不改写历史或正在使用的 QA 断点。
- 同 ID 不同语境和错误目标窗口被拒绝。
- 已绑定任务恢复 QA，禁止覆盖或重复绑定；无法定位的问题不猜最新窗口。
- 相同 checkpoint 不写入，讲解段落不会成为学习者证据。
- 在其他工作目录运行仍使用指定源项目，较长相关段落不截断。
- 未激活时拒绝创建持久 QA 运行态；普通无课程问答由代理直接处理。

这些是实际 Python 状态行为测试，不是对自然语言自动路由或宿主 UI 的端到端证明。本次未创建真实学习任务做测试，也未改写用户学习项目。下面的交互验收案例为待真人使用验证：

| 输入场景 | 预期行为 |
| --- | --- |
| 主线说“把刚才 context 的问题单独开个问答任务” | 自动准备具体语境、创建/复用任务、目标直接解答并保存答案 |
| 新 QA 任务收到精确问题包 | 读取源项目与包，不重新发现或激活课程 |
| 主线已推进，QA 继续旧问题 | 以旧快照解释，核对源码变化，不改写主线断点 |
| 只问“这个语法是什么” | 直接解释，无强制新任务或测验 |
| 要求练习 | 有边界和验收标准，分级提示 |

官方 Skill 校验和源/发现/本机副本一致性在发布时另行检查。
